...
static PaStream *outputStream = NULL;
...
const char* key_to_string(int code) {
    ...
    static char buf[8];
    if (code >= 0x41 && code <= 0x5A) { snprintf(buf, sizeof(buf), "%c", code); return buf; }
    if (code >= 0x30 && code <= 0x39) { snprintf(buf, sizeof(buf), "%c", code); return buf; }
...
char* hotkey_to_string(Hotkey hk, char *out, size_t out_size) {
    ...
    for (int i=0; i<hk.mod_count; i++) {
        strncat(out, key_to_string(hk.modifiers[i]), out_size - strlen(out) - 1);
        strncat(out, "+", out_size - strlen(out) - 1);
    }
    strncat(out, key_to_string(hk.main_key), out_size - strlen(out) - 1);
    return out;
}
...
Hotkey parse_hotkey(const char *str) {
    ...
    while (token) {
        ...
        if (code == VK_CONTROL || code == VK_SHIFT || code == VK_MENU) hk.modifiers[hk.mod_count++] = code;
        else if (code) hk.main_key = code;
        token = strtok(NULL, "+");
    }
    return hk;
}
...
void print_status() {
    ...
    printf("%sSample rate%s: %s%d%s\n", BLUE, RESET, CYAN, cfg.sample_rate, RESET);
    printf("%sFrames/buf%s: %s%d%s\n", BLUE, RESET, CYAN, cfg.frames_per_buffer, RESET);

    printf("%s-%s-%s-%s-%s-%s-%s-%s-%s-%s-%s-%s-%s-%s-%s-%s-%s-\n%s", 
        AQUA, BLUE, AQUA, BLUE, AQUA, BLUE, AQUA, BLUE, AQUA, BLUE, AQUA, BLUE,AQUA, BLUE,AQUA, BLUE,AQUA, BLUE, RESET);
    

    printf("%sMic key%s: %s%s\n", AQUA, RESET, CYAN, hotkey_to_string(cfg.mute_key, buf1, sizeof(buf1)));
    printf("%sVoice key%s: %s%s\n", AQUA, RESET, CYAN, hotkey_to_string(cfg.voice_mute_key, buf2, sizeof(buf2)));
...
#endif
{
    short *buffer = (short*)malloc(sizeof(short) * cfg.frames_per_buffer);
    if (!buffer) return 0;
    for (;;) {
        if (Pa_ReadStream(inputStream, buffer, cfg.frames_per_buffer) != paNoError) continue;
        if (!mic_muted) {
...
#endif
{
    ...
    for (;;) {
        ...
        socklen_t slen = (socklen_t)sizeof(srcaddr);
        int n = recvfrom(sock, (char*)buffer, sizeof(short)*cfg.frames_per_buffer, 0,
                         (struct sockaddr*)&srcaddr, &slen);
        if (n <= 0) continue; // для UDP ок — просто ждём дальше
        if (!voice_muted) {
...
#ifdef _WIN32
DWORD WINAPI key_thread(LPVOID arg)
#else
...
int main() {
    ...
    servaddr.sin_port = htons((unsigned short)cfg.server_port);
    if (inet_pton(AF_INET, cfg.server_ip, &servaddr.sin_addr) != 1) {
        fprintf(stderr, "inet_pton failed for ip: %s\n", cfg.server_ip);
        return 1;
    }

    if (Pa_OpenDefaultStream(&inputStream, 1, 0, paInt16,
                             cfg.sample_rate, cfg.frames_per_buffer, NULL, NULL) != paNoError ||
        Pa_OpenDefaultStream(&outputStream, 0, 1, paInt16,
                             cfg.sample_rate, cfg.frames_per_buffer, NULL, NULL) != paNoError) {
        fprintf(stderr, "Failed to open PortAudio streams\n");
        return 1;
    }
